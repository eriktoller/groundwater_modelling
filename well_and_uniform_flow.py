...
if __name__ == "__main__":
    ...
    phi = (
        lambda x, y: -x * qx
        - y * qy
        + np.real(well_phi(x, y, x0, y0, q, r) + well_phi(x, y, x1, y1, -q, r))
    )
    psi = (
        lambda x, y: x * qy
        - y * qx
        + np.imag(well_phi(x, y, x0, y0, q, r) + well_phi(x, y, x1, y1, -q, r))
    )
    ...
    contour_flow_net(
        xrange, yrange, phi_func=phi, psi_func=psi, levels=50, num_points=1000
    )
    plt.axis(
        "equal"
